...
import type { LumalLayoutTabItem, LumalTabStyle } from './types'
...
const props = withDefaults(defineProps<{
  ...
  visible?: boolean
  wheelable?: boolean
  /** 独立使用 LumalTabs（非 LumalLayout 内嵌）时启用原生 Fullscreen API 最大化。 */
...
function handleTabMouseDown(event: MouseEvent, tab: LumalLayoutTabItem): void {
...
function ensureSortable(): void {
  const list = tabsViewportRef.value?.querySelector<HTMLElement>('.lumal-tabs__list')
  ...
  sortableInstance = Sortable.create(list, {
    animation: 160,
    draggable: '.lumal-tabs__item',
    filter: '.is-not-draggable',
    ghostClass: 'lumal-tabs__item--ghost',
    chosenClass: 'lumal-tabs__item--chosen',
    onEnd: (event) => {
...
/***********************最大化*********************/
/** 应用内内容最大化（LumalLayout 内嵌）：仅由父组件通过事件统一控制 UI 显隐。 */
...
function handleDocumentClick(event: MouseEvent): void {
  ...
  if (target.closest?.('.lumal-tabs-context-menu') || target.closest?.('[data-tab-menu-trigger]')) {
    return
  }
  closeAllMenus()
}
...
<template>
  ...
  >
    <button
      ...
      class="lumal-tabs__tool"
      type="button"
      aria-label="向左滚动标签"
      :disabled="scrollIsAtLeft"
      @click="scrollTabs('left')"
    >
      <LumalIcon name="lumal:chevron-left" :size="16" />
    </button>

    <div
      ref="tabsViewportRef"
      class="lumal-tabs__viewport"
      @scroll="syncScrollState"
      @wheel="handleWheel"
    >
      <TransitionGroup name="lumal-tab" tag="div" class="lumal-tabs__list" role="tablist" aria-label="页面标签">
        <div
          v-for="(tab, index) in tabs"
          :key="tab.path"
          class="lumal-tabs__item"
          :class="{
        ...
        >
          ...
          >
            <LumalIcon
              ...
              class="lumal-tabs__pin"
              name="lumal:pin"
              :size="14"
            />
            <LumalIcon v-else-if="showIcon && tab.icon" :name="tab.icon" :size="14" />
            <span class="lumal-tabs__label" :title="tab.title">{{ tab.title }}</span>
          </button>
          <button
            v-if="canCloseTab(tab.path)"
            class="lumal-tabs__close"
            type="button"
            :aria-label="`关闭${tab.title}`"
            @click="closeTabAction(tab.path)"
          >
            <LumalIcon name="lumal:close" :size="14" />
          </button>
        </div>
      </TransitionGroup>
    </div>
    ...
    >
      <LumalIcon name="lumal:chevron-right" :size="16" />
    </button>

    <button
      v-if="showRefresh"
      class="lumal-tabs__tool lumal-tabs__tool--refresh"
      type="button"
      aria-label="刷新当前页面"
    ...
    >
      <LumalIcon name="lumal:refresh" :size="16" />
    </button>
    ...
    >
      <LumalIcon name="lumal:more" :size="16" />
    </button>

    <button
      v-if="showMaximize"
      class="lumal-tabs__tool lumal-tabs__tool--maximize"
      type="button"
      :aria-label="isMaximized ? '退出内容最大化' : '内容最大化'"
      data-action="maximize-content"
      @click="toggleMaximize"
    >
      <LumalIcon :name="isMaximized ? 'lumal:restore' : 'lumal:maximize'" :size="16" />
    </button>

    <Teleport to="body">
      <div
        v-if="contextMenu.visible || moreMenu.visible"
        class="lumal-tabs-context-menu"
        role="menu"
...
.lumal-tabs {
  ...
  width: 100%;
  min-width: 0;
  height: var(--lumal-tabbar-height);
  border-bottom: 1px solid var(--el-border-color-lighter);
  background: var(--el-bg-color);
}

.lumal-tabs__viewport {
  flex: 1 1 auto;
  min-width: 0;
  overflow-x: auto;
  overflow-y: hidden;
...
.lumal-tabs__viewport::-webkit-scrollbar {
  display: none;
}
...
.lumal-tabs__item {
  ...
  align-items: center;
  border-radius: var(--lumal-radius-small) var(--lumal-radius-small) 0 0;
...
.lumal-tabs__tab:focus-visible,
.lumal-tabs__tool:focus-visible,
.lumal-tabs__close:focus-visible {
...
.lumal-tabs__tool:hover:not(:disabled) {
  color: var(--el-text-color-primary);
  background: var(--el-fill-color-light);
}

.lumal-tabs__tool:disabled {
  cursor: default;
  opacity: 0.35;
}

.lumal-tabs__item--ghost {
  opacity: 0.4;
}

.lumal-tabs__item--chosen {
  box-shadow: inset 0 0 0 1px var(--el-color-primary-light-5);
}

/***********************Chrome 风格*********************/
.lumal-tabs.is-style-chrome .lumal-tabs__item.is-active {
  position: relative;
...
.lumal-tabs.is-style-chrome .lumal-tabs__item.is-active::before,
.lumal-tabs.is-style-chrome .lumal-tabs__item.is-active::after {
  content: '';
  position: absolute;
  bottom: 0;
  width: 8px;
...
.lumal-tabs.is-style-chrome .lumal-tabs__list > .lumal-tabs__item.is-active + .lumal-tabs__item .lumal-tabs__tab::before {
  display: none;
}

.lumal-tabs.is-style-chrome .lumal-tabs__list > .lumal-tabs__item + .lumal-tabs__item .lumal-tabs__tab::before {
  content: '';
  position: absolute;
  top: 12px;
  bottom: 10px;
...
.lumal-tabs.is-style-chrome .lumal-tabs__list > .lumal-tabs__item.is-active + .lumal-tabs__item .lumal-tabs__tab::before,
.lumal-tabs.is-style-chrome .lumal-tabs__list > .lumal-tabs__item + .lumal-tabs__item.is-active .lumal-tabs__tab::before {
  display: none;
}

/***********************Plain 风格*********************/
.lumal-tabs.is-style-plain .lumal-tabs__item.is-active {
  color: var(--el-color-primary);
  border-bottom: 2px solid var(--el-color-primary);
}

.lumal-tabs.is-style-plain .lumal-tabs__list > .lumal-tabs__item + .lumal-tabs__item {
  border-left: 1px solid var(--el-border-color-lighter);
}

/***********************Card 风格*********************/
.lumal-tabs.is-style-card .lumal-tabs__item {
  border: 1px solid transparent;
  border-radius: var(--lumal-radius-small);
}

.lumal-tabs.is-style-card .lumal-tabs__item.is-active {
...
.lumal-tabs.is-style-card .lumal-tabs__item:not(.is-active) {
  background: var(--el-fill-color-light);
}

.lumal-tabs.is-style-card .lumal-tabs__list {
  gap: 4px;
  padding-top: 4px;
}

/***********************Brisk 风格*********************/
.lumal-tabs.is-style-brisk .lumal-tabs__item.is-active {
  color: var(--el-color-primary);
}

.lumal-tabs.is-style-brisk .lumal-tabs__item.is-active::after {
  content: '';
  position: absolute;
  bottom: 4px;
  left: 14px;
  ...
  animation: lumal-tab-brisk-underline 160ms var(--lumal-easing-standard);
}

@keyframes lumal-tab-brisk-underline {
  from {
...
/***********************拖拽光标与固定项视觉*********************/
.lumal-tabs__item.is-pinned .lumal-tabs__tab {
  cursor: default;
}

.lumal-tabs__item.is-not-draggable {
  cursor: default;
}
...
@media (max-width: 768px) {
  ...
  .lumal-tabs__tool--refresh,
  .lumal-tabs__tool--maximize {
    display: none;
  }
}
...
/***********************TransitionGroup 动画*********************/
.lumal-tab-enter-active,
.lumal-tab-leave-active {
  transition:
    opacity 160ms var(--lumal-easing-standard),
    transform 160ms var(--lumal-easing-standard);
}

.lumal-tab-enter-from {
  opacity: 0;
...
.lumal-tab-move {
  transition: transform 160ms var(--lumal-easing-standard);
}
...
<style lang="scss">
.lumal-tabs-context-menu {
  position: fixed;
  z-index: var(--lumal-z-context-menu);
  display: grid;
  min-width: 168px;
  padding: 4px;
  border: 1px solid var(--el-border-color-lighter);
  border-radius: var(--lumal-radius-base);
  background: var(--el-bg-color-overlay);
  box-shadow: var(--lumal-shadow-base);
  transform-origin: top left;
  animation: lumal-tab-menu-in 140ms var(--lumal-easing-standard);
}

@keyframes lumal-tab-menu-in {
  from {
    opacity: 0;
...
.lumal-tabs-context-menu button {
  display: flex;
  align-items: center;
  gap: 8px;
  padding: 8px 12px;
...
.lumal-tabs-context-menu button:hover:not(:disabled) {
  color: var(--el-text-color-primary);
  background: var(--el-fill-color-light);
}

.lumal-tabs-context-menu button:disabled {
  color: var(--el-text-color-placeholder);
  cursor: not-allowed;
}

.lumal-tabs-context-menu button:focus-visible {
  outline: 2px solid var(--el-color-primary);
  outline-offset: -2px;
}

.lumal-tabs-context-menu__divider {
  height: 1px;
  margin: 4px 6px;
