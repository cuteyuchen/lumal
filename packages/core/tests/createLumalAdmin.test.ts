...
import { createLumalAdmin } from '../src'
import { createPermissionStore, usePermissionStore } from '../src/permission'

/***********************测试组件*********************/
function createRootComponent(name = 'RootForCreateLumalAdminTest') {
...
describe('createLumalAdmin', () => {
  ...
  it('支持默认组件、按名称选择和关闭全局组件注册', () => {
    const allComponents = createLumalAdmin({
      components: true,
      rootComponent: createRootComponent('RootForAllComponentsTest'),
    })
    const selectedComponents = createLumalAdmin({
      components: ['LumalPage'],
      rootComponent: createRootComponent('RootForSelectedComponentsTest'),
    })
    const noComponents = createLumalAdmin({
      components: false,
      rootComponent: createRootComponent('RootForNoComponentsTest'),
    })

    expect(allComponents.app.component('LumalPage')).toBeTruthy()
    expect(allComponents.app.component('LumalLayout')).toBeTruthy()
    expect(allComponents.app.component('LumalAccessControl')).toBeTruthy()
    expect(allComponents.app.component('LumalBreadcrumb')).toBeTruthy()
    expect(allComponents.app.component('LumalGlobalSearch')).toBeTruthy()
    expect(selectedComponents.app.component('LumalPage')).toBeTruthy()
    expect(selectedComponents.app.component('LumalLayout')).toBeUndefined()
    expect(noComponents.app.component('LumalPage')).toBeUndefined()
  })
