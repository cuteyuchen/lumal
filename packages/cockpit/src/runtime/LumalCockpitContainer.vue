...
import LumalCockpitWidgetHost from './LumalCockpitWidgetHost.vue'
...
const gridStyle = computed(() => ({
  gridTemplateColumns: props.columns
    .map(column => `calc(${column.width} * var(--lumal-cockpit-x-unit, 1px))`)
    .join(' '),
}))
...
<template>
  ...
  >
    <div v-if="row.mode === 'grid'" class="lumal-cockpit-container__grid" :style="gridStyle">
      <div
        v-for="cell in row.cells"
        :key="cell.id"
        class="lumal-cockpit-container__cell"
        data-cockpit-node="cell"
        :data-cockpit-node-id="cell.id"
        :data-cockpit-side="side"
      >
        <LumalCockpitWidgetHost
          v-if="cell.widget"
          :key="cell.widget.id"
          :layout-id="layoutId"
          :side="side"
          :widget="cell.widget"
        />
        <div v-else class="lumal-cockpit-container__empty" role="status">
          空模块槽
        </div>
      </div>
    </div>
