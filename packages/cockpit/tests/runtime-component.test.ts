...
import LumalCockpit from '../src/runtime/LumalCockpit.vue'
...
describe('lumalCockpit v3 运行时', () => {
  ...
  it('由 activeLayoutId 控制当前布局', async () => {
    const wrapper = mount(LumalCockpit, { props: { config: config(), registry: registry(), activeLayoutId: 'layout-b' } })
  ...
  it('普通模块与合并行各自只有一个视觉 Card，合并行切换时保留所有模块实例', async () => {
    ...
    expect(wrapper.findAll('.lumal-cockpit-card')).toHaveLength(2)
    expect(wrapper.get('[data-cockpit-node-id="left-widget"]').findAll('.lumal-cockpit-card')).toHaveLength(1)
    const tabRow = wrapper.get('[data-cockpit-node-id="right-row"]')
    expect(tabRow.findAll('.lumal-cockpit-card')).toHaveLength(1)
    expect(tabRow.findAll('.lumal-cockpit-widget .lumal-cockpit-card')).toHaveLength(0)
    expect(tabRow.findAll('.lumal-cockpit-card__header')).toHaveLength(1)
    expect(tabs.map(tab => tab.text())).toEqual(['甲', '乙'])
    // Tab 首次激活时挂载，访问后使用 v-show 保留实例状态。
    expect(wrapper.findAll('.stub-widget')).toHaveLength(2)
    expect(tabRow.findAll('.lumal-cockpit-container__tabpanel')).toHaveLength(1)
    await tabs[1].trigger('click')
    expect(tabs[1].attributes('aria-selected')).toBe('true')
    const panels = tabRow.findAll('.lumal-cockpit-container__tabpanel')
  ...
  it('模块标题按显式标题、注册表 label、type 依次回退', () => {
    ...
    expect(wrapper.get('[data-instance-id="explicit-widget"] .lumal-cockpit-card__title').text()).toBe('显式标题')
    expect(wrapper.get('[data-instance-id="registry-widget"] .lumal-cockpit-card__title').text()).toBe('注册标题')
    expect(wrapper.get('[data-instance-id="type-widget"] .lumal-cockpit-card__title').text()).toBe('unknown-type')
  })
  ...
  it('允许自定义 cardComponent 完整替换默认 Card，并透传内容与 Tab 切换', async () => {
    ...
    expect(wrapper.findAll('.lumal-cockpit-card')).toHaveLength(0)
  ...
  it('design 模式点击模块发出 v3 节点事件', async () => {
    const wrapper = mount(LumalCockpit, { props: { config: config(), registry: registry(), renderMode: 'design' } })
  ...
  it('中心内容先渲染为底层，左右区域随后作为覆盖层', () => {
    ...
    const bodyChildren = wrapper.find('.lumal-cockpit-canvas__body').element.children
    expect(bodyChildren[0].classList.contains('lumal-cockpit-canvas__center')).toBe(true)
    expect(bodyChildren[1].classList.contains('lumal-cockpit-canvas__left')).toBe(true)
    expect(bodyChildren[2].classList.contains('lumal-cockpit-canvas__right')).toBe(true)
  })

  it('支持由消费方选择 vwvh 适配模式', () => {
    const wrapper = mount(LumalCockpit, { props: { config: config(), registry: registry(), viewportMode: 'vwvh' } })
    expect(wrapper.attributes('data-viewport-mode')).toBe('vwvh')
    expect(wrapper.find('.lumal-cockpit-canvas').attributes('data-viewport-mode')).toBe('vwvh')
    const stageStyle = wrapper.find('.lumal-cockpit-canvas__stage').attributes('style')
    expect(stageStyle).toContain('100vw')
    expect(stageStyle).toContain('--lumal-cockpit-x-unit')
    expect(stageStyle).toContain('--lumal-cockpit-y-unit')
    expect(wrapper.find('.lumal-cockpit-region').attributes('style')).toContain('--lumal-cockpit-x-unit')
    expect(wrapper.find('.lumal-cockpit-container__grid').attributes('style')).toContain('--lumal-cockpit-x-unit')
  })
})
