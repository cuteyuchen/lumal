...
<template>
  <div
    ref="rootRef"
    class="dv-scroll-board lumal-scroll-board"
  ...
  >
    <div
      v-if="header.length"
      class="header lumal-scroll-board__header"
      role="row"
    ...
    <div class="rows lumal-scroll-board__viewport" :style="{ height: `${rowsHeight}px` }">
      <div
        ...
        :key="entry.renderKey"
        class="row-item lumal-scroll-board__row"
      ...
      >
        <div v-if="hasCustomRows && entry.modernRow" class="ceil lumal-scroll-board__slot">
        ...
        <div
          ...
          v-else
          :key="columnIndex"
          class="ceil lumal-scroll-board__cell"
          role="cell"
...
<style scoped>
.dv-scroll-board.lumal-scroll-board {
  position: relative;
  display: block;
  width: 100%;
  height: 100%;
...
.dv-scroll-board .lumal-scroll-board__slot {
  width: 100%;
  padding: 0;
}
...
.dv-scroll-board[data-mode="table"] .row-item:focus-visible {
  outline: 2px solid var(--lumal-datav-focus-ring, #35c8ff);
  outline-offset: -2px;
}
