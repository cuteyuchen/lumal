...
<template>
  <div
    ref="rootRef"
    class="dv-scroll-ranking-board lumal-scroll-ranking-board"
  ...
  >
    <div class="lumal-scroll-ranking-board__viewport">
      <div
        v-for="entry in renderItems"
        :key="entry.renderKey"
        class="row-item lumal-scroll-ranking-board__row"
        role="listitem"
...
<style scoped>
.dv-scroll-ranking-board.lumal-scroll-ranking-board {
  width: 100%;
  height: 100%;
  min-width: 0;
  min-height: 0;
...
.dv-scroll-ranking-board .lumal-scroll-ranking-board__viewport {
  display: block;
  height: 100%;
  overflow: hidden;
}
...
.dv-scroll-ranking-board .lumal-scroll-ranking-board__slot {
  display: block;
  width: 100%;
  height: auto;
  min-width: 0;
...
.dv-scroll-ranking-board .ranking-column {
  ...
  overflow: visible;
  background: transparent;
  border-bottom: 2px solid color-mix(in srgb, var(--lumal-ranking-color) 50%, transparent);
  border-radius: 0;
}
...
.dv-scroll-ranking-board .inside-column {
  ...
  margin-bottom: 2px;
  overflow: hidden;
  background-color: var(--lumal-ranking-color);
  border-radius: 1px;
}
...
.dv-scroll-ranking-board .shine {
  ...
  animation: lumal-ranking-shine 3s ease-in-out infinite alternate;
}
