...
<template>
  <main class="luma-admin-page">
    <LumaPage title="字典分类" description="维护字典的分类、编码和用途；具体键值请前往“字典项”。" :loading="loading">
      <template #actions>
    ...
    <ElDialog v-model="dialogVisible" append-to-body class="luma-admin-dialog" :title="formMode === 'edit' ? '编辑字典分类' : '新增字典分类'" width="720px">
      <LumaSchemaForm v-model="formModel" :mode="formMode" :schemas="schemas" :columns="2" show-actions submit-text="保存分类" @submit="save" />
    </ElDialog>
  </main>
</template>
